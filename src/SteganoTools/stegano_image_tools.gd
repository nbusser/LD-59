...
func init():
	if _is_stegano_image():
		_rendered_image.material.set_shader_parameter(
			"stegano_texture_sampler_green", Globals.get_current_cipher().image_texture_filter_green
		)
		_rendered_image.material.set_shader_parameter(
			"stegano_texture_sampler_red", Globals.get_current_cipher().image_texture_filter_red
		)
